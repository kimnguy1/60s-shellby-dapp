...
describe("CRY-37 wallet hotfix regression checks", () => {
  ...
  test("regression: manual connect keeps local wallet state when backend sync is unavailable", () => {
    expect(page).toContain("setWalletConnected(normalized)");
    expect(page).toContain("if (err instanceof ApiError)");
    expect(page).toContain("Local demo mode is active until backend sync recovers.");
  });

  test("regression: upload can degrade to local demo mode when API is unreachable", () => {
    expect(page).toContain("Uploaded locally in demo mode; backend sync is pending.");
    expect(page).toContain("id: crypto.randomUUID()");
  });
});
